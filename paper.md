...
- `research_timeline.cli` — Typer-based CLI: commands `init`, `log`, `list`, `export`, `validate`.
...
- Storage: one JSON document, validated against `schema/timeline.schema.json` (JSON Schema draft-07).
...
- Metrics and evidence: free-form key/value pairs attached to an event (e.g., `z_score_combined`, `mi_shared`, `shots`, `backend`, `job_ids`, `git_commit`, `data_links`, `code_links`), so that quantitative results of a physics experiment can be anchored to the event.
- Exporters: LaTeX table (`--format latex`), Markdown, standalone HTML, and schema.org JSON-LD (`@type: ResearchProject`).
- Engineering trade-off: **simplicity over power** — no database, no cloud, no lock-in; a single file readable by any JSON parser, diffable in `git`, durable across decades.
